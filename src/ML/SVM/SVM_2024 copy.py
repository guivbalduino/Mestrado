...
def train_and_predict_svm(collection_name, hoje, param_c, param_ep):
    ...
    svm_model = SVR(kernel='rbf', C=param_c, epsilon=param_ep)
    ...
    base_dir = f"./previsoes/2024/SVM/{sanitize_directory_name(hoje)}/{sanitize_directory_name(collection_name)}"
    os.makedirs(base_dir, exist_ok=True)

    # Salvar modelo
    model_path = os.path.join(base_dir, f"{sanitize_directory_name(collection_name)}_svm_model.joblib")
    ...
    plt.title(f"Previsão de Temperatura - SVM - {collection_name}")
    plt.legend()
    ...
    plt.plot(X_test_dates, y_pred, label="Previsões - SVM", color='red')
    plt.xlabel("Data")
    plt.ylabel("Temperatura (°C)")
...
def save_model_info(collection_name, model_params, eval_metrics, tempo_modelagem, tempo_armazenamento, hoje, quantidade_dados_utilizados, model_path):
    ...
    print("Informações da modelagem armazenadas no MongoDB.")

# Executar o código para todas as coleções que começam com "fusao_"
if __name__ == "__main__":
    # Parâmetros para teste
    C_values = [10, 50, 100, 500]  # Valores para o parâmetro C
    epsilon_values = [0.01, 0.1, 0.5, 1]  # Valores para o parâmetro epsilon
    ...
    collections = get_fusao_collections()  # Todas as coleções "fusao_"
