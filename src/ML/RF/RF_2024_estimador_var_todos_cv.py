...
def load_data_from_mongo(
    collection_name, start_date="2024-02-14", end_date="2024-09-02"
):
...
def train_and_predict_rf(
    ...
    data = load_data_from_mongo(
        collection_name, start_date="2024-02-13", end_date="2024-09-03"
    )
    ...
    model_path = os.path.join(
        base_dir, f"{sanitize_directory_name(collection_name)}_rf_model.joblib"
    )
    ...
    plt.plot(X_test_dates, y_test.values, label="Actual", color="blue")
    plt.plot(X_test_dates, y_pred, label="Predicted", color="red")
    plt.xlabel("Date")
    plt.ylabel("Hourly Total Precipitation (mm)")
    plt.title(f"Precipitation Prediction - Random Forest - {collection_name}")
    plt.legend()
    plt.xticks(rotation=45, ha="right")
    plt.tight_layout()
    plt.savefig(os.path.join(base_dir, "precipitation_prediction.png"), dpi=300)
    plt.close()

    # Gráfico de Métricas de Avaliação
    plt.figure(figsize=(10, 6))  # Aumentei o tamanho para caber mais uma barra
    metrics = ["MAE", "MSE", "R2", "CV MAE"]  # Adicionei 'CV MAE'
    values = [mae, mse, r2, cv_mae]  # Adicionei cv_mae
    colors = ["blue", "orange", "green", "purple"]  # Adicionei uma cor para 'CV MAE'
    ...
    plt.title(f"Model Evaluation - {collection_name}")
    plt.ylabel("Value")
    plt.tight_layout()
    plt.savefig(
        os.path.join(base_dir, "model_evaluation.png"), bbox_inches="tight", dpi=300
    )
    plt.close()

    # Gráfico de Resíduos
    plt.figure(figsize=(10, 5))
    plt.scatter(X_test_dates, residuals, color="red", label="Residuals")
    plt.axhline(y=0, color="black", linestyle="--", label="Zero")
    plt.xlabel("Date")
    plt.ylabel("Residual (Error)")
    plt.title(f"Model Residuals - {collection_name}")
    plt.legend()
    plt.xticks(rotation=45, ha="right")
    plt.tight_layout()
    plt.savefig(os.path.join(base_dir, "model_residuals.png"), dpi=300)
    plt.close()
    ...
    plt.plot(
        ...
        label="All Actual Data",
        color="blue",
        alpha=0.5,
    )
    plt.plot(X_test_dates, y_pred, label="Predictions - Random Forest", color="red")
    plt.xlabel("Date")
    plt.ylabel("Hourly Total Precipitation (mm)")
    plt.title(f"All Data and Predictions - {collection_name}")
    plt.legend()
    plt.xticks(rotation=45, ha="right")
    plt.tight_layout()
    plt.savefig(os.path.join(base_dir, "all_data_with_predictions.png"), dpi=300)
    plt.close()

    # Gráfico de Dados Reais com Previsões (apenas onde há previsões)
    plt.figure(figsize=(10, 5))
    plt.plot(
        X_test_dates,
        y_test.values,
        label="Actual with Predictions",
        color="blue",
        marker="o",
        linestyle="None",
    )
    ...
    plt.xlabel("Date")
    plt.ylabel("Hourly Total Precipitation (mm)")
    plt.title(f"Actual Data and Predictions - {collection_name}")
    plt.legend()
    plt.xticks(rotation=45, ha="right")
    plt.tight_layout()
    plt.savefig(os.path.join(base_dir, "actual_data_with_predictions.png"), dpi=300)
    plt.close()

    return mae, mse, r2, cv_mae
...
colecoes_desejadas_completas = [
    'fusao_temp_arima_es_sktime_2024-12-13_02:28-04',
    'fusao_temp_prophet_2024-12-13_00:42-50',
    'fusao_hier_hdbscan_pca_2024-12-13_00-10-19',
    'fusao_hier_kmeans_pca_2024-12-13_00-10-54'
]
...
def get_fusao_collections():
    client = MongoClient("localhost", 27017)
    db = client["dados"]
    todas_as_collections = db.list_collection_names()
    # Filtrar apenas as coleções que estão na lista desejada (com timestamp)
    return [col for col in todas_as_collections if col in colecoes_desejadas_completas]
...
def train_and_predict_rf(collection_name, hoje, n_estimators, max_depth, min_samples_leaf, min_samples_split, criterion, bootstrap, random_state):
    ...
    # Gráfico de Previsões vs. Valores Reais
    plt.figure(figsize=(10, 5))
    plt.plot(X_test_dates, y_test.values, label="Real", color='blue', linestyle='None')  # linha azul contínua
    ...
    plt.title(f"Previsão de Precipitação - {collection_name[:-20]}")
    plt.legend()
    plt.xticks(rotation=45, ha='right')
    plt.tight_layout()
    ...
    plt.title(f"Avaliação do Modelo - {collection_name[:-20]}")
    plt.ylabel("Valor")
    plt.tight_layout()
    ...
    plt.title(f"Resíduos do Modelo - {collection_name[:-20]}")
    plt.legend()
    plt.xticks(rotation=45, ha='right')
    plt.tight_layout()
    ...
    plt.xlabel("Data")
    plt.ylabel("Precipitação Total Horária (mm)")
    plt.title(f"Todos os Dados com Previsões - {collection_name[:-20]}")
    plt.legend()
    plt.xticks(rotation=45, ha='right')
    plt.tight_layout()
    ...
    plt.xlabel("Data")
    plt.ylabel("Precipitação Total Horária (mm)")
    plt.title(f"Dados Reais e Previsões - {collection_name[:-20]}")
    plt.legend()
    plt.xticks(rotation=45, ha='right')
    plt.tight_layout()
