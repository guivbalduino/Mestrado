...
# Parâmetros ARIMA
start_params = None
arima_order_options = [(1, 1, 1), (2, 1, 1), (1, 0, 1)]   # Exemplos de ordens para ARIMA
freq_resample_options = ["H", "D",'W']  # Frequências de reamostragem
interpolacao_options = [True]  # Interpolação ou não
seasonal_order_options = [(0, 0, 0, 0)]  # Exemplos de ordens sazonais
method_options = ["lbfgs"]  # Métodos de otimização
maxiter_options = [50, 100]  # Exemplos de números máximos de iterações
scoring_options = ["mse", "mae"]  # Métodos de pontuação para avaliação do modelo
trend_options = ['add', 'mul']  # Tipos de tendência
with_intercept_options = [False]  # Considerar ou não intercepto
enforce_stationarity_options = [False]  # Estacionariedade forçada
enforce_invertibility_options = [False]  # Invertibilidade forçada
simple_differencing_options = [False]  # Diferenças simples
mle_regression_options = [True, False]  # Regressão MLE
hamilton_representation_options = [False, True]  # Representação Hamiltoniana
concentrate_scale_options = [False, True]  # Concentrar escala
...
def run_temp_sk_arima_ex():

    # Loop aninhado para percorrer diferentes combinações de parâmetros ARIMA
    for arima_order in arima_order_options:
                        ...
                        for maxiter in maxiter_options:
                            for scoring in scoring_options:
                                for trend in trend_options:
                                    for with_intercept in with_intercept_options:
                                        for enforce_stationarity in enforce_stationarity_options:
                                            for enforce_invertibility in enforce_invertibility_options:
                                                for simple_differencing in simple_differencing_options:
                                                    for mle_regression in mle_regression_options:
                                                        for hamilton_representation in hamilton_representation_options:
                                                            for concentrate_scale in concentrate_scale_options:
