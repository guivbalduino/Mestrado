...
def aplicar_arima(df, coluna, order, exog, dates, freq, missing):
    y = df[coluna]
...
def run_temp_stats_arima():
# Definições dos parâmetros
    freq_resample_options = ["H"]  # Exemplos de frequências de reamostragem
    interpolacao_options = [True]  # Interpolação ou não
    order_options = [(1, 1, 1), (2, 1, 1), (1, 0, 1)]   # Exemplos de ordens ARIMA
    exog_options = [None]  # Exogênicas de exemplo (ajustar conforme necessário)
    dates_options = [None]  # Exemplos de datas
    freq_options = ["H"]  # Frequência de séries temporais
    missing_options = ["none"]  # Métodos de lidar com dados ausentes
                    ...
                    for dates in dates_options:
                        for freq in freq_options:
                            for missing in missing_options:
                                ...
                                nome_colecao = "fusao_temp_arima_statsmodels_" + data_hora_atual.strftime("%Y-%m-%d_%H:%M-%S")
                                ...
                                for coluna in ['temperature_C', 'humidity_percent', 'pressure_hPa',"PRECIPITAÇÃO TOTAL, HORÁRIO (mm)"]:
                                    previsoes = aplicar_arima(df_resampled, coluna, order, exog, dates, freq, missing)
