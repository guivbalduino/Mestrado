...
def run_hier_agglomerative():
    # Parâmetros do Mini-batch
    batch_size = 10000  # Define o tamanho do lote para processamento


    # Parâmetros do Agglomerative Clustering
    n_clusters_options = [5, 10, 20, 100]  # Exemplo de diferentes números de clusters
    linkage_options = ['ward', 'average', 'complete', 'single']  # Métodos de ligação para Agglomerative Clustering
    ...
    for n_clusters in n_clusters_options:
        for linkage in linkage_options:
            for n_components in n_components_options:
                ...
                nome_colecao = f"fusao_hier_aglom_{linkage}_pca_minibatch_" + data_hora_atual.strftime("%Y-%m-%d_%H-%M-%S")
                ...
                df_cluster_final['timestamp'] = timestamps.iloc[:len(df_cluster_final)].reset_index(drop=True)
                ...
                info_fusao = {
                    "nome_fusao": f"agglomerative_{linkage}_pca_minibatch",
                ...
                #time.sleep(70)
    print("===Acabou hier agglomerative===")
