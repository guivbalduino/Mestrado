...
def run_hier_birch():
    # Parâmetros do BIRCH
    branching_factor_options = [50, 100, 200]  # Exemplos de diferentes fatores de ramificação
    n_clusters_options = [None]         # Exemplos de diferentes números de clusters (None para estimar automaticamente)
    threshold_options = [0.1, 0.5, 1.0]       # Exemplos de diferentes limiares
    compute_labels_options = [True]     # Exemplos de configuração para cálculo de labels

    # Parâmetros do PCA
    n_components_options = [2, 3, 4]  # Exemplos de diferentes números de componentes principais

    # Laços aninhados para percorrer diferentes combinações de parâmetros
    for branching_factor in branching_factor_options:
        for n_clusters in n_clusters_options:
            for threshold in threshold_options:
                for compute_labels in compute_labels_options:
                    for n_components in n_components_options:
                        ...
                        nome_colecao = "fusao_hier_birch_pca_" + data_hora_atual.strftime("%Y-%m-%d_%H-%M-%S")
                        ...
                        # Realizar o clustering BIRCH nos dados reduzidos pelo PCA
                        ...
                        print("Fusão hierárquica BIRCH com PCA concluída e resultados armazenados na coleção:", nome_colecao)
                        print("Modelo PCA salvo em:", pca_path)
                        #time.sleep(70)
    print("===Acabou hier birch===")