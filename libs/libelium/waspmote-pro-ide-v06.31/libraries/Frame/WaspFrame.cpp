...
int8_t WaspFrame::addSensor(uint8_t type, int value)
{
	...
	if(_mode == ASCII)
	{
		...
		if (_boot_version >= 'G')
		{
            ...
            else if (_frameType == INFORMATION_FRAME_CITIES_PRO)
            {
                strcpy_P(name, (char*)pgm_read_word(&(CITIES_PRO_TABLE[type])));
            }
            else
            {
	...
	else
	{
		...
		if (_boot_version >= 'G')
		{
			...
			{
				config = (uint8_t)pgm_read_word(&(CITIES_PRO_TYPE_TABLE[type]));
			}
			else
			{
...
int8_t WaspFrame::addSensor(uint8_t type, double value)
{
	...
	if (_boot_version >= 'G')
	{
		...
		{
			numDecimals = (uint8_t)pgm_read_word(&(CITIES_PRO_DECIMAL_TABLE[type]));
		}
		else
		{
...
int8_t WaspFrame::checkFields(uint8_t type, uint8_t typeVal, uint8_t fields)
{
	...
	if (_boot_version >= 'G')
	{
		...
		{
			nfields = (uint8_t)pgm_read_word(&(CITIES_PRO_FIELD_TABLE[type]));
		}
		else
		{
