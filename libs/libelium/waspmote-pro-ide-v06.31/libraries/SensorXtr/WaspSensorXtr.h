...
/*!
 * \struct sensorATMOS14 Vector
 * \brief Struct to store data of the ATMOS sensor
 */

struct sensorATMOS14Vector
{
...
/*!
 * \struct sensorSO421Vector
 * \brief Struct to store data of the SO421 sensor
 */
...
/*!
 * \class ATMOS_14
 * \brief class for ATMOS sensor
 */
class ATMOS_14
{
	public:
		// constructor
		ATMOS_14(uint8_t _socket);

		sensorATMOS14Vector sensorATMOS14;
...
/*!
 * \class Apogee_SO421
 * \brief class for SO421 sensor
 */
class Apogee_SO421
{
	public:
		// constructor
		Apogee_SO421(uint8_t _socket);

		sensorSO421Vector sensorSO421;
...
class weatherStation : public WaspUART
{
	public:
		//! Constructor
		weatherStation();
		...
		uint8_t read();
		uint8_t enterCommand(char* str);
		uint8_t enterSetupMode();
		uint8_t exitSetupMode();
	...
	private:
		...
		#define GILL_RATE 19200
		#define GILL_UART_SIZE 512
		uint8_t class_buffer[GILL_UART_SIZE];
};
...
/*!
 * \class Apogee_SQ100X
 * \brief class for SQ100 sensor
 */
class Apogee_SQ100X
{
	public:
		//! Constructor
		Apogee_SQ100X(uint8_t _socket);
