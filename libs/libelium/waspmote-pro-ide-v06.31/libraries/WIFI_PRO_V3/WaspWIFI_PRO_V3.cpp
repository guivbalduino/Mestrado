...
void WaspWIFI_v3::dataTreatment(char data[])
{
...
uint8_t WaspWIFI_v3::configureStation(char* ssid, char* password, uint8_t autoconnect)
{
	...
	sprintf_P(command, (char*)pgm_read_word(&(table_wifi_v3[4])), _ssid, password, autoconnect);
...
uint8_t WaspWIFI_v3::tcpConnect(uint8_t tcp_session_id)
{
	...
	// send command
	uint8_t status = sendCommand(command, WIFI_V3_OK, WIFI_V3_CME_ERROR, WIFI_V3_ERROR, 60000);
...
uint8_t WaspWIFI_v3::tcpSendHTTP(uint8_t tcp_session_id, char* data)
{
	...
	if (status == 1)
	{
		...
		// Wait for "HTTP/" (Server response)
		status = waitFor(WIFI_V3_HTTP_RES, WIFI_V3_CME_ERROR, WIFI_V3_ERROR, 10000);
		status = waitFor(" ", WIFI_V3_CME_ERROR, WIFI_V3_ERROR, 10000);
		// Get response status
		waitFor(WIFI_V3_CR_LF,500);
		char* ptr;
		_httpResponseStatus = strtod((char*)_buffer,&ptr);
...
uint8_t WaspWIFI_v3::httpPost(char http_path[], char data[])
{
	...
	// "AT+KHTTPHEADER=<http_session_id>\r"
	sprintf_P((char*)_buffer,(char*)pgm_read_word(&(table_wifi_v3[62])), _httpSessionId);
	uint8_t status = sendCommand((char*)_buffer, WIFI_V3_CONNECT, WIFI_V3_CME_ERROR, WIFI_V3_ERROR, 10000);
	
	
	if (status == 1)
	{
		sprintf(content,"Content-Length: %u\r\n",strlen(data));
		#if DEBUG_WIFI_V3 > 0
			PRINT_WIFI_V3("Data to send: ");
			USB.println(data);
			PRINT_WIFI_V3("Content Length: ");
			USB.println(strlen(data));
		#endif
		printString(content,_uart);
		delay(1000);
		status =sendCommand("+++", WIFI_V3_OK, WIFI_V3_ERROR,10000);
		
		if (status == 1)
		{
			// "AT+KHTTPPOST=<http_session_id>, <request_uri>\r"
			sprintf_P((char*)_buffer, (char*)pgm_read_word(&(table_wifi_v3[28])), _httpSessionId, http_path);
			// send command
			 status = sendCommand((char*)_buffer, WIFI_V3_CONNECT, WIFI_V3_CME_ERROR, WIFI_V3_ERROR, 10000);
		...
		else if (status == 2)
		{
			httpCloseConnection();
			httpDeleteConfig();
			USB.println("HTTP Post Content Error");
			return 2;
		}		
	}
	else if (status == 2)
...
{
	return mqttConfiguration(mqtt_server, client_id, mqtt_port, security, "", "");
}
 ...
 * 
 */
uint8_t WaspWIFI_v3::mqttConfiguration(char mqtt_server[], char client_id[], uint16_t mqtt_port, uint8_t security,char username[],char password[])
{
	char command[100];
	uint8_t error;
	...
	// "AT+KHTTPCFG=,<security>,<mqtt_server>,<mqtt_port>,4,<client_id>,,,,,,,, <username>, <password>]\r"
	sprintf_P(command, (char*)pgm_read_word(&(table_wifi_v3[42])), security, mqtt_server, mqtt_port, client_id,username,password);
	...
	if (status == 1)
	{
		...
		memcpy(_mqttPassword,password,sizeof(_mqttPassword));
		memcpy(_mqttUsername,username,sizeof(_mqttUsername));
		_mqttPort = mqtt_port;
		_mqttSec = security;
...
uint8_t WaspWIFI_v3::mqttGetTopicData(char topic[], uint8_t qos)
{
	...
	error = mqttConfiguration(_mqttServer, _mqttClientID, _mqttPort, _mqttSec,_mqttUsername, _mqttPassword);
...
uint8_t WaspWIFI_v3::mqttPublishTopic(char topic[], uint8_t qos, uint8_t retained, char data[])
{
	...
	sprintf_P(command, (char*)pgm_read_word(&(table_wifi_v3[46])), _mqttSessionId, topic, qos, retained, nData);
